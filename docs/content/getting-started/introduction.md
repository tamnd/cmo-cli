...
cmo is a single binary. It speaks to cmo over plain HTTPS,
shapes the responses into clean records, and gets out of your way. There is
nothing to sign up for and nothing to run alongside it.

## How it is built

- A **library package** (`cmo`) holds the HTTP client and the typed
  data models. It paces requests, sets an honest User-Agent, and retries the
  transient failures any public site throws under load.
- A **domain** (`cmo/domain.go`) declares each operation once on the
...
ant get cmo://page/<path>        # the URI dereference (via a host)
```
...
cmo is a read-only client over data cmo already serves
